...
public class AirportsResource {
	...
	private static Logger logger = LoggerFactory.getLogger(AirportsResource.class);
	 ...
	 * asyncResponse, the asynchronous response has a timeout of 120 seconds, the time
	 * elapses will produce HTTP status 503 SERVICE_UNAVAILABLE, exception handle
	 * will remote service will be reported to asyncResponse.
	 * 
	 * The HeaderParam fly-host could be used specify a new URI to provide the JSON data
	 * 
	 * @param response
	 * @HeaderParam flyhost
	 */
	...
	public void getAirports(@Suspended final AsyncResponse response, @HeaderParam("fly-host") String flyhost) {
		...
		if (flyhost != null && flyhost.length() > 0 && validateHTTP_URI(flyhost)) {
			...
			aHost = "https://api.flightstats.com/flex/airports/docs/v1/lts/samples/Airports_response.json";
		}
	
		Future<String> futureResponse = client.target(aHost).request().async()
				.get(new InvocationCallback<String>() {
					...
					public void completed(String value) {
		        		...
		        		try {
		        			...
		        			JsonNode jsonNode2 = jsonNode1.get("airports");
		        			ObjectReader reader = mapper.readerFor(new TypeReference<List<Airport>>() {
		        			});
		        			List<Airport> list = reader.readValue(jsonNode2);
		        			if (list.size() > 200) {
		        				response.resume(Response.ok(jsonNode2).type(MediaType.APPLICATION_JSON).build());
		        			}
		        		} catch (JsonParseException e) {
		        			logger.error(e.getMessage());
		        			response.resume(Response.status(Response.Status.BAD_REQUEST.getStatusCode(), "JSON JsonParseException").type(MediaType.APPLICATION_JSON).build());
		        		} catch (JsonMappingException e) {
		        			logger.error(e.getMessage());
		        			response.resume(Response.status(Response.Status.BAD_REQUEST.getStatusCode(), "JSON JsonMappingException").type(MediaType.APPLICATION_JSON).build());
		        		} catch (IOException e) {
		        			logger.error(e.getMessage());
